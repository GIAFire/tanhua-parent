package com.itheima.autoconfig.lvwang;
...
public class AliyunGreenTemplate {
     ...
     * @throws Exception
     */
    private Map<String, String> greenTextScan(String content) throws Exception {
...
//                    设置默认错误返回内容
     ...
     * 阿里云图片内容安全
     */
    private Map<String, String> imageScan(List<String> imageList) throws Exception {
        IClientProfile profile = DefaultProfile
                .getProfile("cn-shanghai", greenProperties.getAccessKeyID(), greenProperties.getAccessKeySecret());
    ...
    public Boolean checkText(String content) {
        try {
            Map<String, String> map = greenTextScan(content);
            if (map != null) {
    ...
    public Boolean checkImage(List<String> imageList) {
        try {
            Map<String, String> map = imageScan(imageList);
            if (map != null) {
                if (map.get("suggestion").equals("pass")) {
                    return true;
                }
